...
Suppose that $\C$ is a clan (Def. 2.7 in the notes) with a choice of path objects for each object, satisfying that each $\epsilon : \Path Y \to Y \times Y$ is in $\D$. Show that $\epsilon_f$ is in $\D$. (Hint: Use Problem 1 (a), (c) and (d))
