...
\begin{lemma}
    ...
    Nice path objects give a reflexive and symmetric relations.
...
\begin{proof}
    The reflection $\lambda_{\Delta_f}$ is given by construction.

    This is symmetric because we can form the following lifting problem
    \[
         \begin{tikzcd}
             A \ar[r,"\lambda"] \ar[d,"\lambda"] & \Path A \ar[d,"\rho"]
             \\ 
             \Path A \ar[r,"\tau \rho"] \ar[ur,dotted] & A \times A
         \end{tikzcd}
    \]
    where $\lambda \in \mathcal L$ and $\rho \in \mathcal R$, so that there is a solution. Here, $\tau : A \times A \to A \times A$ refers to the `twist' map that switches the two copies of $A$.

    For transitivity, we form the following lifting problem.
    \[
         \begin{tikzcd}
             \Path A \ar[r,equal ] \ar[d,"\lambda'"] & \Path A \ar[d,"\rho"]
             \\ 
             \Path A \times_\bullet \Path A \ar[r,""] & A \times A
         \end{tikzcd}
    \]
    where $\Path A \times_\bullet \Path A$ is the following pullback, i.e., it is the object of composable paths.
    \[
         \begin{tikzcd}
            \Path A \times_\bullet \Path A \ar[r," "] \ar[d," "] \pullback & \Path A \ar[d,"\pi_1 \rho"]
             \\ 
             \Path A \ar[r,"\pi_0 \rho"] & A
         \end{tikzcd}
    \]
    In the lifting problem, the map $\Path A \times_\bullet \Path A \to A \times A$ is the projection to the endpoints not used in the pullback.

    There are two choices for $\lambda'$ in the lifting problem. For instance, we can take $\lambda'$ to be induced by 
    \[
         \begin{tikzcd}
            \Path A \ar[r,dotted] \ar[dr, equal] \ar[rr, bend left, "\lambda \pi_1 \rho"] & \Path A \times_\bullet \Path A \ar[r," "] \ar[d," "] \pullback & \Path A \ar[d,"\pi_1 \rho"]
             \\ 
             & \Path A \ar[r,"\pi_0 \rho"] & A
         \end{tikzcd}
    \]
    Notice that this is the pullback of $\lambda : A \to \Path A$ along the projection $\Path A \times_\bullet \Path A \to \Path A$, which we claim is in $\mathcal R$. Thus, since $\mathcal L$ is stable under pullback along $\mathcal R$, we have that $\lambda'$ is in $\mathcal L$ and thus we have a solution to our lifting problem.

    To see that $\Path A \times_\bullet \Path A \to \Path A$ is in $\mathcal R$, note that either projection $A \times A \to A$ is in $\mathcal R$, as the pullback of $A \to *$. Thus, the compositions $\Path A \to A \times A \to A$ is in $\mathcal R$. Thus, both projections $\Path A \times_\bullet \Path A \to \Path A$ are in $\mathcal R$.
\end{proof}
...
\end{lemma}
\begin{proof}
    We show the first statement. It is immediate that every $f \in \mathcal L$ has this property.

    Conversely, consider an $f$ with this property, and let $s_f$ denote a solution to the lifting problem in the statement. To show that it is in $\mathcal L$, we show that it lifts against all $r \in \mathcal R$. Thus consider a lifting problem of $f$ against $r$.
    \[
         \begin{tikzcd}
             X \ar[r,"x"] \ar[d,"f"] & R_0 \ar[d,"r"]
             \\ 
             Y \ar[r,"y"] & R_1
         \end{tikzcd}
    \]
    If we factor $f$ and add in $s_f$ to this square, we find the following.
    \[
         \begin{tikzcd}
             X \ar[r,"x"] \ar[d,"\lambda_f"] & R_0 \ar[dd,"r"]
             \\ 
             Mf \ar[d, "\rho_f"] \ar[ur,dotted]
             \\
             Y \ar[r,"y"] \ar[u,"s_f", dashed, bend left] & R_1
         \end{tikzcd}
    \]
    Note that the solid diagram commutes, but $s_f$ does not commute with everything.
    We can view this as a lifting problem of $\lambda_f \in \mathcal L$ against $r \in \mathcal R$ to find a dotted arrow. One can check the composition we now see in the diagram $Y \to R_0$ constitutes a solution to our original lifting problem.
